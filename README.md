...
below is the json content to be used instead of calling the content from the s3 bucket. also please create the lambda first. after that the module.

once the lambda is created that lambda should be used in invoking which we are suing in create connect module. if there is any other lambda replace it with the one we are creating.
...
also please write everything, do not miss anything. I want all servies and all the content in the json inculded in the yaml template
